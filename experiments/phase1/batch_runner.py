...
def aggregate_statistics(
    ...
    aggregated = {
        ...
        'final_energy': {
            'mean': float(np.mean([e for e in final_energy_avgs if e > 0])) if any(e > 0 for e in final_energy_avgs) else 0,
            'std': float(np.std([e for e in final_energy_avgs if e > 0])) if any(e > 0 for e in final_energy_avgs) else 0
        }
    }
...
def print_comparison(aggregated_a: Dict[str, Any], aggregated_b: Dict[str, Any]):
    ...
    if aggregated_a and aggregated_b:
        print("\nSurvival Time:")
        print(f"  Condition A: {aggregated_a['survival_time']['mean']:.2f} ± {aggregated_a['survival_time']['std']:.2f}")
        print(f"  Condition B: {aggregated_b['survival_time']['mean']:.2f} ± {aggregated_b['survival_time']['std']:.2f}")
        
        print("\nFinal Alive Count:")
        print(f"  Condition A: {aggregated_a['final_alive']['mean']:.2f} ± {aggregated_a['final_alive']['std']:.2f}")
        print(f"  Condition B: {aggregated_b['final_alive']['mean']:.2f} ± {aggregated_b['final_alive']['std']:.2f}")
        
        if 'dual_process' in aggregated_b:
            print("\nDual-Process Metrics (Condition B):")
            print(f"  Veto Rate: {aggregated_b['dual_process']['veto_rate_mean']:.2%} ± {aggregated_b['dual_process']['veto_rate_std']:.2%}")
...
def main():
    ...
    if args.condition in ['B', 'both']:
        condition_b_stats = run_condition(
            ...
            condition='B',
            num_runs=args.num_runs,
            start_seed=args.start_seed + (args.num_runs if args.condition == 'both' else 0)
        )
    
    # Aggregate statistics
    aggregated_a = aggregate_statistics(condition_a_stats, 'A') if condition_a_stats else {}
    aggregated_b = aggregate_statistics(condition_b_stats, 'B') if condition_b_stats else {}
