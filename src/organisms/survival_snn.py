...
class SurvivalSNN(nn.Module, Organism):
    ...
    def forward(self, x: torch.Tensor, num_steps: int = 10) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def spike_to_action(self, spike_train: torch.Tensor, context: Dict[str, Any] = None) -> Action:
        ...
        if winner_idx < 4:
            ...
            if context.get('has_food_here', False):
                return Action.EAT
            elif context.get('has_adjacent_organism', False) and not context.get('low_energy', False):
