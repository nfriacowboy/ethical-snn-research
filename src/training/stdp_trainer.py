...
class STDPTrainer:
            ...
            for pre_idx in range(num_pre):
                ...
                for post_t in post_times:
                    for pre_t in pre_times:
                        ...
                        weight_delta[post_idx, pre_idx] += dw * (1.0 + self.reward_factor * reward)
    ...
    def train_episode(
        ...
        for (pre_spikes, post_spikes), reward in zip(spike_history, reward_history):
            # Update both layers
            stats1 = self.update_weights(pre_spikes, post_spikes, reward, layer_name='fc1')
            stats2 = self.update_weights(post_spikes[:, :post_spikes.shape[1]], 
                                        torch.rand(post_spikes.shape[0], 5), 
                                        reward, layer_name='fc2')
            updates.append((stats1, stats2))
        
        return {
            'total_reward': total_reward,
            'num_updates': len(updates),
            'avg_weight_change': np.mean(self.weight_changes[-len(updates):]) if self.weight_changes else 0.0
        }
