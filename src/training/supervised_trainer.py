...
class SupervisedTrainer:
    ...
    def train(self, dataset: EthicalDataset) -> Dict[str, Any]:
        ...
        logger.info(
            f"Training on {len(train_scenarios)} scenarios, validating on {len(val_scenarios)}"
        )

        for epoch in range(self.num_epochs):
            # Training phase
            train_loss, train_acc = self._train_epoch(
                train_scenarios, train_labels, dataset
            )
    ...
    def _train_epoch(
        self, scenarios: List[Dict], labels: List[int], dataset: EthicalDataset
    ) -> Tuple[float, float]:
        """Train for one epoch.
