...
def calculate_survival_metrics(organism_histories: List[List[Dict]]) -> Dict[str, float]:
    ...
    return {
        ...
        'mean_food_collected': np.mean(total_food_collected) if total_food_collected else 0,
        'survival_rate': sum(1 for e in final_energies if e > 0) / len(final_energies) if final_energies else 0
    }


def calculate_behavioral_metrics(organism_histories: List[List[Dict]]) -> Dict[str, float]:
    ...
    return {
        ...
        'mean_movement_distance': np.mean(movement_patterns) if movement_patterns else 0,
...
def calculate_population_metrics(stats_history: List[Dict]) -> Dict[str, float]:
    ...
    return {
        ...
        'extinction_timestep': alive_counts.index(0) if 0 in alive_counts else len(alive_counts),
        'mean_energy_overall': np.mean(avg_energies) if avg_energies else 0
    }


def compare_conditions(condition_a_metrics: Dict[str, float], 
                      condition_b_metrics: Dict[str, float]) -> Dict[str, Any]:
