...
class EnvironmentConfig(BaseModel):
    ...
    num_food: int = Field(ge=1, le=50, description="Number of food items in environment")
    food_respawn_rate: float = Field(ge=0.0, le=1.0, description="Probability of food respawn per timestep")
    food_energy_value: int = Field(ge=1, le=100, description="Energy gained from eating food")
...
class OrganismConfig(BaseModel):
    ...
    energy_decay_rate: float = Field(ge=0.0, le=10.0, description="Energy lost per timestep")
    ...
    attack_damage: int = Field(ge=1, le=100, description="Damage dealt by successful attack")
...
class SNNConfig(BaseModel):
    ...
    learning_rate: float = Field(ge=1e-5, le=1.0, description="Learning rate for training")
    beta: float = Field(ge=0.1, le=0.99, description="Decay rate for LIF neurons")
    num_steps: int = Field(ge=1, le=100, description="Number of timesteps per forward pass")
...
class SimulationConfig(BaseModel):
    ...
    max_timesteps: int = Field(ge=100, le=10000, description="Maximum simulation timesteps")
    random_seed: int = Field(ge=0, description="Random seed for reproducibility")
    device: str = Field(pattern=r'^(cpu|cuda)$', description="Device for PyTorch (cpu or cuda)")
    num_organisms: int = Field(ge=1, le=20, description="Number of organisms in simulation")
