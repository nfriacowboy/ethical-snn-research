...
class SimulationRunner:
        ...
        for i in range(self.num_organisms):
            ...
            if self.condition == "A":
                ...
                organism = SurvivalSNN(
                    organism_id=i, position=position, initial_energy=100.0
                )
            elif self.condition == "B":
                # Condition B: Dual-process (SNN-S + SNN-E)
                organism = DualProcessOrganism(
                    organism_id=i, position=position, energy=100.0
                )
            else:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        logger.info(f"Organisms: {self.num_organisms}")
        logger.info(f"Grid size: {self.grid.grid_size}x{self.grid.grid_size}")
        logger.info(f"Max timesteps: {self.max_timesteps}")
        ...
        for t in range(self.max_timesteps):
            ...
            if alive_count == 0:
                logger.info(f"All organisms dead at timestep {t}")
                break
        ...
        logger.info(f"Final timestep: {self.timestep}")
    ...
    def step(self) -> None:
        ...
        for organism in self.organisms:
            if organism.alive:
                ...
                # Check for death
                if not organism.alive and organism.organism_id not in [
                    d[0] for d in self.death_times
                ]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "organisms": {
                ...
                "avg_survival_time": (
                    np.mean([org.age for org in dead_organisms])
                    if dead_organisms
                    else self.timestep
                ),
            },
            "energy": {
                "final_avg": (
                    np.mean([org.energy for org in alive_organisms])
                    if alive_organisms
                    else 0
                ),
                "final_std": (
                    np.std([org.energy for org in alive_organisms])
                    if alive_organisms
                    else 0
                ),
            },
            "environment": {"final_food_count": len(self.grid.food_positions)},
        }

        # Add condition-specific statistics
        if self.condition == "B":
            dual_organisms = [
                org for org in self.organisms if hasattr(org, "veto_count")
            ]
            stats["dual_process"] = {
                ...
                "avg_veto_rate": np.mean(
                    [
                        (
                            ...
                            if (org.veto_count + org.approval_count) > 0
                            else 0
                        )
