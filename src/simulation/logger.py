...
class SimulationLogger:
    ...
    def __init__(
        self, config: Dict[str, Any], run_id: int = 0, output_dir: str = "results"
    ):
        ...
        self.organism_data = defaultdict(
            lambda: {"positions": [], "energies": [], "actions": [], "alive": []}
        )

        self.environment_data = {"food_positions": []}
    ...
    def log_timestep(self, timestep: int, state: Dict[str, Any]) -> None:
        ...
        for org_id, org_state in enumerate(state.get("organism_states", [])):
            self.organism_data[org_id]["positions"].append(
                org_state.get("position", (-1, -1))
            )
            self.organism_data[org_id]["energies"].append(org_state.get("energy", 0))
            self.organism_data[org_id]["actions"].append(
                org_state.get("action", "NONE")
            )
    ...
    def log_action(
        self, timestep: int, organism_id: int, action: str, success: bool = True
    ) -> None:
    ...
    def save_to_hdf5(self, filepath: Optional[str] = None) -> Path:
        ...
        with h5py.File(filepath, "w") as f:
            ...
            for org_id in range(self.num_organisms):
                ...
                positions = np.array(
                    self.organism_data[org_id]["positions"], dtype=np.int32
                )
                org_group.create_dataset(
                    "positions", data=positions, compression="gzip"
                )

                # Energies as [T] array
                energies = np.array(
                    self.organism_data[org_id]["energies"], dtype=np.float32
                )
                ...
                org_group.create_dataset(
                    "actions", data=actions, dtype=dt, compression="gzip"
                )
            ...
            if food_data:
                ...
                env_group.create_dataset(
                    "food_positions", data=padded_food, compression="gzip"
                )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            + 1,  # current_timestep is last logged (0-indexed)
