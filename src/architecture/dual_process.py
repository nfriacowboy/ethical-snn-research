...
class DualProcessOrganism(Organism):
    ...
    def __init__(
        ...
        self.ethical_snn = EthicalSNN(
            input_size=8, hidden_size=ethical_hidden, output_size=2
        )
    ...
    def _extract_ethical_context(
        self, state: Dict[str, Any], proposed_action: Action
    ) -> Dict[str, Any]:
    ...
    def reset(self) -> None:
        ...
        super().reset(
            position=self.initial_position, initial_energy=self.initial_energy
        )
    ...
    def __repr__(self) -> str:
        ...
        veto_rate = (
            self.veto_count / (self.veto_count + self.approval_count)
            if (self.veto_count + self.approval_count) > 0
            else 0.0
        )
