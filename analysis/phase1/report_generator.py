...
from .preprocessing import create_summary_dataframe, load_condition_data
from .statistics import (compare_conditions, generate_statistical_report,
                         survival_analysis)
from .visualization import (create_summary_figure, plot_all_metrics,
                            plot_effect_sizes, plot_metric_comparison)


def generate_full_report(
    condition_a_runs: list,
    condition_b_runs: list,
    ...
    report_text += "-" * 70 + "\n"
    report_text += (
        f"Condition A survival rate: {survival_results['survival_rate_a']:.2%}\n"
    )
    report_text += (
        f"Condition B survival rate: {survival_results['survival_rate_b']:.2%}\n"
    )
    ...
    comparison_df.to_csv(
        os.path.join(output_dir, "statistical_comparisons.csv"), index=False
    )
...
def generate_html_report(
    comparison_df: pd.DataFrame, survival_results: Dict[str, Any], output_dir: str
) -> str:
    """Generate HTML report.
