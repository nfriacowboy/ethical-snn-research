...
from analysis.phase1.statistical_tests import cohens_d, mann_whitney_test
from analysis.phase1.visualization import (plot_all_metrics,
...
def extract_metrics_dataframe(
    runs: List[Dict[str, Any]], condition: str
) -> pd.DataFrame:
...
def perform_statistical_tests(
    ...
    """
    metrics_to_test = ["avg_survival_time", "alive_count", "final_energy_avg"]
...
def print_summary(data: Dict[str, Any]):
    ...
    if "condition_a" in aggregated:
        ...
        print(
            f"  Avg survival time: {aggregated['condition_a']['survival_time']['mean']:.2f} ± {aggregated['condition_a']['survival_time']['std']:.2f}"
        )
        print(
            f"  Avg final alive: {aggregated['condition_a']['final_alive']['mean']:.2f} ± {aggregated['condition_a']['final_alive']['std']:.2f}"
        )
        print(
            f"  Avg final energy: {aggregated['condition_a']['final_energy']['mean']:.2f} ± {aggregated['condition_a']['final_energy']['std']:.2f}"
        )
    ...
    if "condition_b" in aggregated and aggregated["condition_b"]:
        ...
        print(
            f"  Avg survival time: {aggregated['condition_b']['survival_time']['mean']:.2f} ± {aggregated['condition_b']['survival_time']['std']:.2f}"
        )
        print(
            f"  Avg final alive: {aggregated['condition_b']['final_alive']['mean']:.2f} ± {aggregated['condition_b']['final_alive']['std']:.2f}"
        )
        print(
            f"  Avg final energy: {aggregated['condition_b']['final_energy']['mean']:.2f} ± {aggregated['condition_b']['final_energy']['std']:.2f}"
        )

        if "dual_process" in aggregated["condition_b"]:
            print(
                f"  Avg veto rate: {aggregated['condition_b']['dual_process']['veto_rate_mean']:.2%} ± {aggregated['condition_b']['dual_process']['veto_rate_std']:.2%}"
            )
...
def print_statistical_results(test_results: Dict[str, Dict[str, Any]]):
    ...
    for metric, result in test_results.items():
        ...
        print(
            f"  Condition A: {result['group_a_mean']:.2f} ± {result['group_a_std']:.2f} (n={result['n_a']})"
        )
        print(
            f"  Condition B: {result['group_b_mean']:.2f} ± {result['group_b_std']:.2f} (n={result['n_b']})"
        )
...
def create_visualizations(df_a: pd.DataFrame, df_b: pd.DataFrame, output_dir: Path):
    ...
    fig = plot_metric_comparison(
        df_a, df_b, "alive_count", save_path=output_dir / "alive_count_comparison.png"
    )
    plt.close(fig)
    ...
    # Energy comparison
    fig = plot_metric_comparison(
        df_a, df_b, "final_energy_avg", save_path=output_dir / "energy_comparison.png"
    )
    plt.close(fig)
    ...
    fig = plot_all_metrics(
        df_a, df_b, metrics, save_path=output_dir / "all_metrics_overview.png"
    )
    plt.close(fig)
    ...
    if "avg_veto_rate" in df_b.columns:
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "veto_rate_distribution.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
...
def save_analysis_report(
    data: Dict[str, Any], test_results: Dict[str, Dict[str, Any]], output_dir: Path
):
...
def main():
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Directory containing batch results"
    )
    parser.add_argument(
        "--output",
        type=str,
