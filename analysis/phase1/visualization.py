...
def plot_metric_comparison(
    df_a: pd.DataFrame, df_b: pd.DataFrame, metric: str, save_path: Optional[str] = None
):
...
def plot_all_metrics(
    ...
    for i, metric in enumerate(metrics):
        data = pd.DataFrame(
            {
                "Condition": ["Survival-Only"] * len(df_a)
                + ["Dual-Process"] * len(df_b),
                metric: list(df_a[metric]) + list(df_b[metric]),
            }
        )
...
def plot_timeseries_comparison(
    timeseries_a: List[pd.DataFrame],
...
def plot_effect_sizes(comparison_df: pd.DataFrame, save_path: Optional[str] = None):
    ...
    ax.set_title(
        "Effect Sizes: Dual-Process vs Survival-Only\n" "Positive = Dual-Process higher"
    )
