...
def compare_conditions(df_a: pd.DataFrame, df_b: pd.DataFrame, 
    ...
    if len(results) > 1:
        corrected_p, corrected_alpha = bonferroni_correction(results_df['p_value'].tolist())
...
def generate_statistical_report(df_a: pd.DataFrame, df_b: pd.DataFrame,
    ...
    for _, row in comparison_results.iterrows():
        report.append(f"\n{row['metric'].upper()}:")
        report.append(f"  Condition A: M = {row['group_a_mean']:.3f}, "
                     f"Mdn = {row['group_a_median']:.3f}, SD = {row['group_a_std']:.3f}")
        report.append(f"  Condition B: M = {row['group_b_mean']:.3f}, "
                     f"Mdn = {row['group_b_median']:.3f}, SD = {row['group_b_std']:.3f}")
        report.append(f"  Mann-Whitney U = {row['statistic']:.3f}, p = {row['p_value']:.4f} "
                     f"{'*' if row['significant'] else ''}")
        report.append(f"  Cohen's d = {row['cohens_d']:.3f} ({row['effect_size_interpretation']})")
        
        if 'p_value_corrected' in row:
            report.append(f"  Corrected p = {row['p_value_corrected']:.4f} "
