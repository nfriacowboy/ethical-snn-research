...
def load_condition_data(run_ids: List[int], results_dir: str = "results") -> List[Dict[str, Any]]:
...
def extract_timeseries_data(run_data: Dict[str, Any]) -> pd.DataFrame:
    ...
    for ts_data in timesteps:
        ...
        if alive_count > 0:
            avg_energy = sum(org['energy'] for org in organism_states if org['alive']) / alive_count
            avg_age = sum(org['age'] for org in organism_states if org['alive']) / alive_count
        else:
            avg_energy = 0
            avg_age = 0
