...
class TestWeightUpdates:
    ...
    def test_update_weights_basic(self):
        ...
        stats = trainer.update_weights(pre_spikes.float(), post_spikes.float(), reward=1.0)
    ...
    def test_update_with_zero_reward(self):
        ...
        stats = trainer.update_weights(pre_spikes.float(), post_spikes.float(), reward=0.0)
    ...
    def test_update_with_negative_reward(self):
        ...
        stats = trainer.update_weights(pre_spikes.float(), post_spikes.float(), reward=-5.0)
...
class TestEpisodeTraining:
    ...
    def test_train_episode_basic(self):
        ...
        # Create episode
        spike_history = [
            (torch.rand(10, 8) > 0.8, torch.rand(10, 30) > 0.8)
            for _ in range(5)
        ]
        reward_history = [1.0, 2.0, 0.0, 3.0, 1.5]
        
        stats = trainer.train_episode(
            [(s[0].float(), s[1].float()) for s in spike_history],
            reward_history
        )
...
class TestRewardModulation:
    ...
    def test_higher_reward_bigger_update(self):
        ...
        stats1 = trainer1.update_weights(pre_spikes.float(), post_spikes.float(), reward=1.0)
        stats2 = trainer2.update_weights(pre_spikes.float(), post_spikes.float(), reward=10.0)
