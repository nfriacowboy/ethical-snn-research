...
def print_progress_bar(
    iteration: int, total: int, prefix: str = "", length: int = 50
) -> None:
...
def print_results(condition: str, stats: Dict[str, Any]) -> None:
    ...
    print(
        f"  Duration: {stats['final_timestep']} timesteps ({stats['elapsed_time']:.2f}s)"
    )
    print(
        f"  Organisms Alive: {Fore.GREEN if stats['organisms']['alive'] > 0 else Fore.RED}"
        f"{stats['organisms']['alive']}/{stats['organisms']['total']}{Style.RESET_ALL}"
    )
    print(
        f"  Average Survival Time: {stats['organisms']['avg_survival_time']:.1f} timesteps"
    )
    ...
    if condition == "B" and "dual_process" in stats:
        total_decisions = (
            stats["dual_process"]["total_vetoes"]
            + stats["dual_process"]["total_approvals"]
        )
...
def compare_conditions(stats_a: Dict[str, Any], stats_b: Dict[str, Any]) -> None:
    ...
    print(
        f"  Difference: {Fore.GREEN if diff > 0 else Fore.RED}{diff:+.1f} timesteps "
    ...
    if diff > 10:
        print(
            "  ✅ Ethical constraints appear to HELP survival (possibly by reducing conflict)"
        )
    elif diff < -10:
        print(
            "  ⚠️  Ethical constraints appear to HINDER survival (trade-off hypothesis)"
        )
    else:
        print(
            "  ℹ️  Minimal difference - ethical constraints have little survival impact"
        )
