...
class SimulationAnimator:
    ...
    def record_frame(
        self, organisms: List[Any], food_positions: List[Tuple[int, int]], timestep: int
    ) -> None:
    ...
    def create_animation(
        ...
        food_scatter = ax_main.scatter(
            [], [], c="green", marker="s", s=100, label="Food"
        )
        organism_scatter = ax_main.scatter(
            [],
            [],
            c=[],
            cmap="plasma",
...
def run_with_recording(
    ...
    organisms = []
    organism_class = (
        DualProcessOrganism if config["condition"] == "B" else SingleProcessOrganism
    )
    ...
    for i in range(config["num_organisms"]):
        ...
        org = organism_class(
            organism_id=i, initial_energy=100.0, max_energy=100.0, position=position
        )
    ...
    for t in range(config["max_timesteps"]):
        ...
        for org in organisms:
            if org.is_alive():
                # Get state
                local_state = env.get_local_state(
                    org.position, org.energy, org.max_energy
                )
                ...
                if action < 4:  # Movement
                    ...
                    dy, dx = moves[action]
                    new_pos = (
                        (org.position[0] + dy) % config["grid_size"],
        ...
        if (t + 1) % 100 == 0:
            ...
            print(
                f"Timestep {t+1}/{config['max_timesteps']} | Alive: {alive}/{len(organisms)}"
            )
    ...
    stats = {
        ...
        "organisms": {
            ...
            "avg_survival_time": np.mean(
                [org.death_time if org.death_time else t for org in organisms]
            ),
        },
    }
...
def main():
    ...
    # Load config
    config_path = (
        Path(__file__).parent.parent / "experiments" / "phase1" / "config_phase1.yaml"
    )
    ...
    if args.output is None:
        ...
        args.output = str(
            output_dir / f"simulation_condition_{args.condition}_seed_{args.seed}.mp4"
        )
